...
env.parallel                     = True
hstore_home                     = '/root/h-store-master'
hostfile                     = 'cluster.cfg'
clients                     = []
servers                     = ["192.168.0.12","192.168.0.13"]
...
def updateCfg():
    ...
    with open("fab.cfg", "r") as reader:
        ...
        print "[updateCfg]:  Updating \"" + default_propt + "\" in All Sites (by fab.cfg)....\n"
        with open(default_propt, "r+") as modifier:
            filestr = modifier.read()
            ...
            if len(clients) == 0:
                clients_str = 'localhost'
            else:
                clients_str = ','.join(clients) + ',localhost'    
            filestr = _updater(filestr,"client.hosts",clients_str) 
            filestr = _updater(filestr,"global.sshoptions", "-x -i " + env.key_filename)

            modifier.seek(0)
            modifier.truncate()
            modifier.write(filestr)
        print "2"
        execute(_upf, hosts = remote_hosts, path_to_file = default_propt)
